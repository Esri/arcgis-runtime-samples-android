...
class MainActivity : AppCompatActivity() {
    ...
    // the route ahead denoted with a magenta color line
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // authentication with an API key or named user
        // is required to access basemaps and other location services
        ArcGISRuntimeEnvironment.setApiKey(BuildConfig.API_KEY)

        // set the basemap style to be displayed in the layout's MapView
        // and create the graphic overlay
        mapView.apply {
        ...
        navigateButton.setOnClickListener {
            // start the route navigation on button tap
            startNavigation()
        }
        recenterButton.setOnClickListener {
            // change the mapview to use navigation mode on button tap
            mapView.locationDisplay.autoPanMode = LocationDisplay.AutoPanMode.NAVIGATION
        }
        resetButton.setOnClickListener {
            mapView.apply {
                // stop the location data source
                locationDisplay.locationDataSource.stopAsync().addDoneListener {
                    // on completed, set up map to display the route overview
                    setUpMapRoute()
                }
                // clear the map's graphics and create a new one
        ...
        // calculate the route from the starting location and destination
        // and display the route overview
        setUpMapRoute()
    }

    /**
     * Calculates the route from San Diego Convention Center to the Fleet Science Center.
     * Then sets up the route graphics and displays the route overview.
     */
    ...
    private fun startNavigation() {
        ...
        if (routeTask.routeTaskInfo.isSupportsRerouting) {
            ...
            routeTracker?.enableReroutingAsync(reroutingParameters)?.addDoneListener {
                mapView.locationDisplay.addAutoPanModeChangedListener {
                    // turn the recenter button on or off when
                    // the location display changes to or from navigation mode
                    recenterButton.isEnabled =
                        it.autoPanMode != LocationDisplay.AutoPanMode.NAVIGATION
                }
                // create a simulated data source
                val simulatedLocationDataSource = SimulatedLocationDataSource()
                // create simulation parameters starting at the current time,
                // velocity, and horizontal and vertical accuracy.
                ...
                // set the locations to travel using the JSON polylines
                simulatedLocationDataSource.setLocations(
                    Geometry.fromJson(polylineJSON) as Polyline,
                ...
                // add re-routing listeners
                routeTracker?.addRerouteStartedListener(routeStartedListener)
                routeTracker?.addRerouteCompletedListener(routeCompletedListener)
            }
        }
    }
